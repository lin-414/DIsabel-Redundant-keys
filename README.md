...
```text
Documents/My Games/Skyrim Special Edition/SKSE/DIsabel Redundant keys.log
```
...
dist/
└── DIsabel-Redundant-keys-v0.4.0.zip
```
...
- Renamed the plugin to **DIsabel Redundant keys** and moved to a standalone repository.
...
Note for other SKSE hotkey mods: while no pausing menu is open, the suppressed user events arrive renamed to `DIsabelRedundantKeys_Consumed`. Mods that read the physical key codes directly are unaffected.

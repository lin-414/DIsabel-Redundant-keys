...
namespace DisabelRedundantKeys
{
    constexpr std::string_view kConsumedEvent = "DIsabelRedundantKeys_Consumed";
            ...
            RE::BSTEventSource<RE::InputEvent*>*) override
        {
            ...
            for (auto* event = *eventPtr; event; event = event->next) {
                ...
                if (IsSuppressed(button->QUserEvent())) {
                    ...
                    // directly.
                    button->userEvent = kConsumedEvent.data();
                }
            }
...
SKSEPluginLoad(const SKSE::LoadInterface* skse)
{
    ...
    if (path) {
        *path /= "DIsabel Redundant keys.log";
    ...
    logger::info("DIsabel Redundant keys v{} loading", DISABEL_REDUNDANT_KEYS_VERSION);

    auto* messaging = SKSE::GetMessagingInterface();
    if (!messaging || !messaging->RegisterListener(DisabelRedundantKeys::MessageHandler)) {
